...
<pre>
	...
	표준 구성 요소
	1. 정적인 텍스트 : HTML, Javascript, text...
	2. 스크립트 요소 : <% %>
		1) 지시자(Directive) : &lt;%@ 지시자명 속성명="속성값" %&gt;
			: JSP 페이지에 대한 기본설정 또는 환경설정에 사용. 전처리에 사용.
			지시자명 page : JSP 페이지 환경 설정에 사용.
		...
		2) 스크립틀릿(Scriptlet) :
			...
			%>
		3) 표현식(Expression) : <%=new Date() %>, <%=test %> <%-- 표현식은 값을 출력하는 역할 --%>
		4) 선언부(Declaration) : 
		<%!
			// 선언부는 전역변수나 (전역)메소드를 선언.
		void sample(){
...
// 		Java 주석
	...
	3. 기본 객체(내장 객체)
	4. Action tag
	5. EL(표현언어, Expresstion Language)
