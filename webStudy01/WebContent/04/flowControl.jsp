...
<pre>
	1. Request Dispatch : 원본 요청에 대한 정보를 페이지를 이동하는 과정에서 전달해주는 방식.
		1) Forward : 페이지를 이동한 이후에 최종 도착지에서 응답데이터가 전송되는 방식
		<%
// 			RequestDispatcher rd = request.getRequestDispatcher("/05/destination.jsp"); //서버사이드방식으로 절대경로를 설정해야함
// 			rd.forward(request, response);
		%>
		2) Include : 페이지를 이동한 후 도착지에서 생성된 결과데이터(버퍼의 내용)을 
...
// 			rd.include(request, response);
	...
	2. Redirect : 
		...
		4) 최종 응답데이터는 3)에서 발생한 새로운 요청에 대한 응답으로 전송.
